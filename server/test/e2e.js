...
(async () => {
  ...
  let jetonA = '', userA = null;
  {
    const r = await appel('/api/auth/register', {
      methode: 'POST', body: { name: 'Cyril', email: 'Cyril@Exemple.FR', password: 'motdepasse123' }
    });
    eq('inscription 200', r.statut, 200);
    vrai('jeton renvoyé', typeof r.data.token === 'string' && r.data.token.length > 30);
    eq('e-mail normalisé', r.data.user.email, 'cyril@exemple.fr');
    jetonA = r.data.token; userA = r.data.user;
  }
  {
    const dup = await appel('/api/auth/register', {
      methode: 'POST', body: { name: 'X', email: 'cyril@exemple.fr', password: 'motdepasse123' }
    });
    eq('doublon 409', dup.statut, 409);

    const court = await appel('/api/auth/register', {
      methode: 'POST', body: { name: 'X', email: 'b@exemple.fr', password: 'court' }
    });
    eq('mot de passe court 400', court.statut, 400);

    const mail = await appel('/api/auth/register', {
      methode: 'POST', body: { name: 'X', email: 'pas-un-email', password: 'motdepasse123' }
    });
    eq('e-mail invalide 400', mail.statut, 400);
  }
  ...
  console.log('\n── Connexion ──');
  {
    const faux = await appel('/api/auth/login', {
      methode: 'POST', body: { email: 'cyril@exemple.fr', password: 'mauvais-mot-de-passe' }
    });
    eq('mauvais mot de passe 401', faux.statut, 401);
    eq('message neutre', faux.data.error, 'E-mail ou mot de passe incorrect.');

    const inconnu = await appel('/api/auth/login', {
      methode: 'POST', body: { email: 'personne@exemple.fr', password: 'motdepasse123' }
    });
    eq('compte inconnu : même message', inconnu.data.error, 'E-mail ou mot de passe incorrect.');

    const bon = await appel('/api/auth/login', {
      methode: 'POST', body: { email: 'cyril@exemple.fr', password: 'motdepasse123' }
    });
  ...
  console.log('\n── Session ──');
  {
    ...
    eq('me identité', r.data.user.email, 'cyril@exemple.fr');
  ...
  console.log('\n── Cloisonnement ──');
  {
    const b = await appel('/api/auth/register', {
      methode: 'POST', body: { name: 'Autre', email: 'autre@exemple.fr', password: 'motdepasse123' }
    });
  ...
  console.log('\n── Limitation de débit ──');
  {
    ...
    for (let i = 0; i < 40; i++) {
      const r = await appel('/api/auth/login', {
        methode: 'POST', body: { email: 'cyril@exemple.fr', password: 'faux' + i }
      });
